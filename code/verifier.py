...
from learnZono2 import analyze_zono

DEVICE = 'cpu'
INPUT_SIZE = 28

def analyze(net, netName, inputs, eps, true_label):
    if analyze_zono(net, netName, copy.deepcopy(inputs), copy.deepcopy(eps), true_label):
        return True
    elif analyze_zono(net, netName, copy.deepcopy(inputs), copy.deepcopy(eps), true_label, useRandom=True):
        return True
